...
class UsernameTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Positional encoding (increased by n_tokens to account for contexts tokens)
    ...
    def forward(self, action_sequence, browser=None, duration_bucket=None, speed_bucket=None, username=None, training=True):
        ...
        # Create context token embeddings
        context_embs = []
        context_masks = []
        ...
        # Concatenate context tokens at the beginning, then action tokens
        if context_embs:
        ...
        # Global average pooling over sequence length (excluding context tokens)
        # Use mask to ignore padding tokens, but exclude context tokens from pooling
        n_actual_contexts = len(context_embs)  # Actual number of context tokens used in this forward pass
        mask_expanded = mask.squeeze(1).squeeze(1).float()  # (batch_size, n_actual_contexts + seq_len)
        # Only pool over action tokens (skip context tokens at the beginning)
